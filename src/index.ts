  ...
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (request.method === "GET" || request.method === "HEAD") {
      ...
      if (request.method === "GET") {
        ...
        const isIndexPath = url.pathname.slice(1).endsWith("/");

        const getFileKey = isIndexPath ? `${objectName}index.html` : objectName;

        const object = await env.MY_BUCKET.get(getFileKey, {
          range,
